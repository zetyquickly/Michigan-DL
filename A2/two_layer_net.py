...
def nn_forward_backward(
    ...
    ############################################################################
    # Replace "pass" statement with your code
    shifted_scores = scores - scores.max(dim=1)[0].view(-1, 1)
    idx0 = torch.arange(N)
    correct_class_score = shifted_scores[idx0, y].view(-1, 1) # N x 1
    mat_exp = torch.exp(shifted_scores) # N x C
    denom = mat_exp.sum(dim=1).view(N, 1) # N x 1
    loss = torch.mean(-correct_class_score + torch.log(denom))
    # Regularization
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    grads["W1"] = torch.zeros_like(W1)
    grads["b1"] = torch.zeros_like(b1)
    # out1 = X @ W1 + b1
    # hidden = (out1 > 0) * out1
    # scores = hidden @ W2 + b2 
    # L = softmax(scores)
    # diff. softmax
    grad_scores = mat_exp / denom # N x C
    idx0 = torch.arange(N)
    grad_scores[idx0, y] -= 1
    grad_scores = grad_scores / N
    
    grads["b2"] = grad_scores.sum(dim=0)
    grad_mm2 = grad_scores
    grads["W2"] = h1.t() @ grad_mm2  # hidden: N x H, grad_mm2: N x C, res: H x C
    grad_hidden = grad_mm2 @ W2.t() # W2: H x C, grad_mm2: N x C, res: N x H
    grad_out1 = (h1 > 0) * grad_hidden
    grads["b1"] = grad_out1.sum(dim=0)
    grad_mm1 = grad_out1
    grads["W1"] = X.t() @ grad_mm1
...
def find_best_net(
    ...
    stat_dict = {}
    lrs = torch.linspace(0.5, 2, 3)
    regs = torch.linspace(1e-6, 1e-4, 3)
    hsizes = [64, 128, 192]
    for lr in lrs:
        for hs in hsizes:
            for reg in regs:
