...
class Linear(object):
  ...
  def backward(dout, cache):
    ...
    # Ds = x.shape[1:]
    db = dout.sum(dim=0)
    dw = x.view(N, -1).t().mm(dout) # dout: N x M, x: N x D, dw: D x M
    dx = dout.mm(w.t()).reshape(x.shape) # dout: N x M, w: D x M, dx: N x Ds
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx, dw, db
...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    dout1, self.grads['W2'], self.grads['b2'] = Linear.backward(dout2, cache2)
    self.grads['W2'] += self.reg * 2 * self.params['W2']
    _, self.grads['W1'], self.grads['b1'] = Linear_ReLU.backward(dout1, cache1)
    self.grads['W1'] += self.reg * 2 * self.params['W1']
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for idx in range(self.num_layers):
        if idx == 0:
            ...
            in_dim = hidden_dims[idx - 1]
        if idx == self.num_layers - 1:
            out_dim = num_classes
        else:
            out_dim = hidden_dims[idx]
        self.params[f"W{idx+1}"] = weight_scale * torch.randn((in_dim, out_dim), dtype=dtype, device=device)
        self.params[f"b{idx+1}"] = torch.zeros(out_dim, dtype=dtype, device=device)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    for idx in range(self.num_layers - 1):
        out_i, cache_i = Linear_ReLU.forward(inp_i, self.params[f'W{idx+1}'], self.params[f'b{idx+1}'])
        global_cache.append(cache_i)
        if self.use_dropout:
            out_i, cache_i = Dropout.forward(out_i, self.dropout_param)
            global_cache.append(cache_i)
        inp_i = out_i
    scores, cache_last = Linear.forward(inp_i, self.params[f'W{self.num_layers}'], self.params[f'b{self.num_layers}'])
    ...
    # Replace "pass" statement with your code
    self.grads = grads
    loss, dwnstrm_grad = softmax_loss(scores, y)
    for idx in range(self.num_layers, 0, -1):
        loss += self.reg * torch.sum(self.params[f'W{idx}'] ** 2)
        cache = global_cache.pop()
        if idx == self.num_layers:
            dwnstrm_grad, self.grads[f'W{idx}'], self.grads[f'b{idx}'] = Linear.backward(dwnstrm_grad, cache)
        else:
            if self.use_dropout:
                dwnstrm_grad = Dropout.backward(dwnstrm_grad, cache)
                cache = global_cache.pop()
            dwnstrm_grad, self.grads[f'W{idx}'], self.grads[f'b{idx}'] = Linear_ReLU.backward(dwnstrm_grad, cache)
        self.grads[f'W{idx}'] += self.reg * 2 * self.params[f'W{idx}']
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
