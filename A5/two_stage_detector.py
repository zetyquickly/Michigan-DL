...
class ProposalModule(nn.Module):
  ...
  def forward(self, features, pos_anchor_coord=None, \
    ...
    ############################################################################
    # Replace "pass" statement with your code
    out = self.pred_layer(features) # B x (6*A) x 7 x 7
    B, _, H, W = out.shape
    anchor_data = out.reshape(B, self.num_anchors, 6, H, W)
    if mode == "train":
        ...
        conf_scores = torch.cat([pos_extracted[:, 0:2], neg_extracted[:, 0:2]], dim=0)
        offsets = pos_extracted[:, 2:6]
        proposals = GenerateProposal(
            pos_anchor_coord.view(-1, 1, 1, 1, 4), offsets.view(-1, 1, 1, 1, 4), method='FasterRCNN'
        ).squeeze()
    elif mode == "eval":
        conf_scores = anchor_data[:, :, 0:2, :, :]
        offsets = anchor_data[:, :, 2:6, :, :]
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    if mode == 'train':
...
class RPN(nn.Module):
  ...
  def forward(self, images, bboxes, output_mode='loss'):
    ...
    self.prop_module = self.prop_module.train()
    ...
    pos_anchor_idx, negative_anc_ind, GT_conf_scores, GT_offsets, GT_class, \
        activated_anc_coord, negative_anc_coord = ReferenceOnActivatedAnchors(anc_list, bboxes, grid_list, iou_mat, neg_thresh=0.3, method='FasterRCNN')
    conf_scores, offsets, proposals = self.prop_module(features, activated_anc_coord, pos_anchor_idx, negative_anc_ind)
    # loss
  ...
  def inference(self, images, thresh=0.5, nms_thresh=0.7, mode='RPN'):
    ...
    self.prop_module = self.prop_module.eval()
    with torch.no_grad():
        B, _, _, _ = images.shape
        features = self.feat_extractor(images)
        conf_scores, offsets = self.prop_module(features, None, None, None)
        ...
        conf_scores = torch.sigmoid(conf_scores.permute(0, 1, 3, 4, 2).reshape(B, A * H * W, 2))
        proposal_list = proposal_list.reshape(B, A * H * W, 4)
        
        for b_id in range(B):
            thres_id = (conf_scores[b_id][:, 0] > thresh).view(-1)
            nms_id = torchvision.ops.nms(proposal_list[b_id][thres_id, :], conf_scores[b_id][thres_id, 0].view(-1), iou_threshold=nms_thresh)
            final_proposals.append(proposal_list[b_id][thres_id, :][nms_id, :])
            final_conf_probs.append(conf_scores[b_id][thres_id, 0:1][nms_id, :])
