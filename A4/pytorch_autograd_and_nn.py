...
def three_layer_convnet(x, params):
  ...
  scores = F.linear(flatten(F.relu(F.conv2d(F.relu(F.conv2d(x, conv_w1, bias=conv_b1, stride=1, padding=2)), conv_w2, conv_b2, padding=1))), fc_w, fc_b)
  ##############################################################################
  #                                 END OF YOUR CODE                             
  ##############################################################################
  return scores
...
def initialize_three_layer_conv_part2(dtype=torch.float, device='cpu'):
  ...
  conv_b1 = torch.randn((channel_1,), dtype=dtype, device='cuda')
  conv_w2 = nn.init.kaiming_normal_(torch.empty((channel_2, channel_1, kernel_size_2, kernel_size_2,), dtype=dtype, device='cuda'))
  conv_b2 = torch.randn((channel_2,), dtype=dtype, device='cuda')
  fc_w = nn.init.kaiming_normal_(torch.empty((num_classes, channel_2 * H * W,), dtype=dtype, device='cuda'))
  fc_b = torch.randn((num_classes,), dtype=dtype, device='cuda')
  for item in [conv_w1, conv_b1, conv_w2, conv_b2, fc_w, fc_b]:
...
class ThreeLayerConvNet(nn.Module):
  ...
  #   1. Convolutional layer with channel_1 5x5 filters with zero-padding of 2  
    ...
    # Replace "pass" statement with your code
    H = W = 32
    self.conv1 = torch.nn.Conv2d(in_channel, channel_1, kernel_size=5, padding=2, bias=True)
    self.conv2 = torch.nn.Conv2d(channel_1, channel_2, kernel_size=3, padding=1, bias=True)
    self.fc = torch.nn.Linear(channel_2 * H * W, num_classes, bias=True)
    self.conv1.weights = nn.init.kaiming_normal_(torch.empty(channel_1, in_channel, 5, 5))
    self.conv1.weights = nn.init.kaiming_normal_(torch.empty(channel_2, channel_1, 3, 3))
    self.fc.weights = nn.init.kaiming_normal_(torch.empty(num_classes, channel_2 * H * W))
    ############################################################################
    #                           END OF YOUR CODE                            
    ############################################################################
...
def initialize_three_layer_conv_part4():
  ...
  model = nn.Sequential(OrderedDict([
    ('conv1', nn.Conv2d(C, channel_1, kernel_size=kernel_size_1, padding=pad_size_1, bias=True)),
    ('relu1', nn.ReLU()),
    ('conv2', nn.Conv2d(channel_1, channel_2, kernel_size=kernel_size_2, padding=pad_size_2, bias=True)),
    ('relu1', nn.ReLU()),
    ('flatten', Flatten()),
    ('fc1', nn.Linear(channel_2 * H * W, num_classes, bias=True)),
  ]))
...
class PlainBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    # ReLU
    # Convolutional layer with Cout 3x3 filters, zero-padding of 1, and stride 2 if downsampling; otherwise stride 1
    # Spatial Batch normalization
    # ReLU
    # Convolutional layer with Cout 3x3 filters, with zero-padding of 1
...
class ResNet(nn.Module):
  def __init__(self, stage_args, Cin=3, block=ResidualBlock, num_classes=10):
    ...
    self.cnn = nn.Sequential(OrderedDict([
        ("stem", ResNetStem(Cin, stage_args[0][0])),
        *((f"stage{idx+1}", *[ResNetStage(args[0], args[1], args[2], downsample=args[3], block=block)]) for idx, args in enumerate(stage_args))
    ]))
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
  ...
  def forward(self, x):
    ...
    # Replace "pass" statement with your code
    feats = self.cnn(x)
    pooled = F.avg_pool2d(feats, kernel_size=feats.shape[2:])
    scores = self.fc(flatten(pooled))
    ############################################################################
    #                                 END OF YOUR CODE                         #
    ############################################################################
    return scores
...
class ResidualBottleneckBlock(nn.Module):
  def __init__(self, Cin, Cout, downsample=False):
    ...
    # Spatial Batch normalization
    # ReLU
    # Convolutional layer with Cout // 4 1x1 filters, stride 2 if downsampling; otherwise stride 1
    # Spatial Batch normalization
    # ReLU
    # Convolutional layer with Cout // 4 3x3 filters, with zero-padding of 1
    # Spatial Batch normalization
    # ReLU
    # Convolutional layer with Cout 1x1 filters
