...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  print(f"Trying to force {target_y} label")
  for iter in range(max_iter):
      pred = model(X_adv)
      pred_det = pred.detach()
      print('Iteration %d: target score %.3f, max score %.3f' % (iter, pred_det[0, target_y], torch.max(pred_det)))
      loss = pred[0, target_y]
      if (loss - torch.max(pred_det)).abs() < 1e-5:
          break
      loss.backward()
      grad = X_adv.grad.data
      with torch.no_grad():
        X_adv +=  learning_rate * grad / torch.linalg.norm(grad)
      print(X_adv.grad.data.sum().item())
    #   print("grad zeroed")
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    grad_norm = kwargs.pop('grad_norm', False)
    ########################################################################
    ...
    # Replace "pass" statement with your code
    pred = model(img)
    loss = pred[0, target_y] + l2_reg * torch.linalg.norm(img)
    loss.backward()
    grad = img.grad.data 
    with torch.no_grad():
        img +=  learning_rate * grad  / (torch.linalg.norm(grad) if grad_norm else 1.)
    img.grad.data.zero_()
    ########################################################################
    #                             END OF YOUR CODE                         #
    ########################################################################
