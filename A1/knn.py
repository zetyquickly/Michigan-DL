...
def compute_distances_two_loops(x_train, x_test):
  ...
  for idx in range(num_train):
      train_sample = x_train[idx].view(-1)
      for jdx in range(num_test):
          test_sample = x_test[jdx].view(-1)
          dists[idx,jdx] = torch.sqrt(((train_sample - test_sample) ** 2).sum())
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  for idx in range(num_train):
      train_sample = x_train[idx].view(-1)
      test_dists = x_test.view(num_test, -1) - train_sample
      test_dists = torch.sqrt((test_dists ** 2).sum(axis=1))
      dists[idx, :] = test_dists
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  threeSums = torch.sum(torch.square(A).view(num_train, 1, -1), axis=2) - 2 * (A @ B.T) + torch.sum(torch.square(B), axis=1)
  dists = torch.sqrt(threeSums)
  print("salamon")
  # dists = torch.sqrt((torch.square(A.view(num_train, 1, -1) - B).sum(axis=2)))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def predict_labels(dists, y_train, k=1):
  ...
  ##############################################################################
  # Replace "pass" statement with your code
  _, indices = torch.topk(dists, axis=0, largest=False, k=k)
  for idx in range(num_test): 
      label, _ = torch.mode(y_train[indices[:, idx]], axis=0)
      y_pred[idx] = label
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
      ...
      for idx in range(num_folds):
          x_train_loc = torch.cat(x_train_folds[:idx] + x_train_folds[idx+1:])
          x_test_loc = x_train_folds[idx]
          y_train_loc = torch.cat(y_train_folds[:idx] + y_train_folds[idx+1:])
          y_test_loc = y_train_folds[idx]
          classifier = KnnClassifier(x_train_loc, y_train_loc)
          accuracies.append(classifier.check_accuracy(x_test_loc, y_test_loc, k=k))
...
def knn_get_best_k(k_to_accuracies):
  ...
  for k, accs in k_to_accuracies.items():
      if torch.mean(torch.tensor(accs)) > max_acc:
          max_acc = torch.mean(torch.tensor(accs))
          best_k = k
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
